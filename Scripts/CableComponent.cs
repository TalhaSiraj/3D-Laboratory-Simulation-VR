...
public class CableComponent : MonoBehaviour
{
	...
	private float segmentsPerUnit = 1.0f;
	private int segments = 0;
	private float cableWidth = 0.005f;
	...
	void Start()
	{
		Debug.Log("CableScript Start");
		InitCableParticles();
		InitLineRenderer();
	}
	void Awake(){
		Debug.Log("CableScript Awake");
		// InitCableParticles();
		// InitLineRenderer();
	}
	...
	{
		line = AddingScript.getnewBornBaby().gameObject.AddComponent<LineRenderer>();
		line.SetWidth(cableWidth, cableWidth);
		line.SetVertexCount(segments + 1);
		string wireColor_ = AddingScript.getwirecolor();
		if(!wireColor_.Contains("None")){
			if(wireColor_.Contains("positive")){
				GetComponent<Renderer>().material.color = Color.red;
			}else{
				if(wireColor_.Contains("negative")){
					GetComponent<Renderer>().material.color = Color.blue;
				}else{
					if(wireColor_.Contains("spare")){
